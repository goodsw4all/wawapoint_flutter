...
### B. 왜 백업은 JSON인가? (SQLite 직접 백업과의 비교)

SQLite 데이터베이스 파일(`.db`)을 직접 복사하여 백업 파일로 활용하지 않고, 데이터를 JSON 형태로 내보내는 데는 다음과 같은 중요한 아키텍처적 이유가 있습니다.

1. **스키마 마이그레이션 및 하위 호환성 (Schema Migration & Backward Compatibility)**
   - 앱이 업데이트되면서 데이터베이스 테이블 스키마(예: 새로운 컬럼 추가, 필드 타입 변경)가 바뀔 수 있습니다.
   - 구버전의 SQLite 파일(`.db`)을 그대로 덮어써서 복원하면 신버전 앱에서 구조 불일치로 인한 런타임 크래시가 발생할 수 있습니다.
   - JSON 형태로 데이터를 추출하면 백업 파일 내의 `version` 메타데이터를 기반으로, 복원 시점에 최신 스키마 구조에 맞춰 데이터를 변환(Migration)하여 적재할 수 있습니다.

2. **데이터 유효성 검증의 용이성 (Data Validation)**
   - 바이너리 파일인 SQLite `.db` 파일은 전송 도중 손상되었거나 위변조되었을 때, 실제로 데이터베이스를 열어서 쿼리를 해보기 전까지 손상 여부를 검증하기가 어렵습니다.
   - JSON 포맷은 파싱 전/후 단계에서 스키마 구조와 필수 필드(예: `id`, `date`, `amount`)가 정상적으로 채워져 있는지 코드로 즉시 검사할 수 있어 비정상적인 데이터로 인한 오작동을 차단합니다.

3. **크로스 플랫폼 호환성 (Cross-platform Portability)**
   - Flutter는 iOS, Android, Web, Desktop 등 다양한 플랫폼을 빌드 타겟으로 설정합니다.
   - SQLite 라이브러리 및 파일 엔진은 타겟 OS에 따라 물리적 파일 접근 및 드라이버 호환성 이슈가 생길 수 있지만, JSON 텍스트 포맷은 기기나 OS 종류에 구애받지 않고 100% 동일하게 직렬화 및 역직렬화할 수 있습니다.

4. **유연한 데이터 병합 (Data Merging)**
   - 데이터베이스 파일을 직접 덮어쓰면 복원 시 기존에 생성되었던 로컬 데이터가 전부 유실됩니다.
   - 반면 JSON 포맷으로 데이터를 읽어오면 기존 SQLite 레코드와 대조하여 유일한(UUID) 항목만 추가로 삽입(Upsert / Merge)하는 형태의 정밀한 동기화 기능으로 확장할 수 있습니다.

5. **인간 가독성 (Human-Readability) 및 이식성**
   - 백업 데이터가 투명한 텍스트 형식이므로 개발자나 사용자가 직접 백업 파일을 열어서 백업 상태를 손쉽게 진단하고, 다른 데이터 플랫폼으로 데이터를 마이그레이션하기 편리합니다.
