...
*(비유하자면, 앱들이 서로의 장난감을 빼앗지 못하도록 놀이터에서 아이들에게 하나씩 나누어준 독립된 모래놀이 상자(Sandbox)와 같습니다. 다른 상자의 모래를 함부로 만질 수 없으며 오직 자신에게 주어진 모래상자 안에서만 안전하게 모래성을 쌓을 수 있습니다.)*
...
*(비유하자면, 기본키는 중복이 불가능한 주민등록번호와 같습니다. 만약 백업 데이터에 동일한 번호를 가진 다른 사람이 들어온다면, 데이터베이스는 둘 중 누구를 살려야 할지 판단할 수 없거나 기존 데이터를 덮어써 버리는 대참사가 발생합니다. 이를 위해 전 세계에서 유일한 값(UUID)을 발급하여 충돌을 원천 차단하는 것입니다.)*
...
## 3. 초보자를 위한 복원 핵심 용어 사전 🔍

백업 및 복원 파이프라인 흐름도에서 사용된 핵심 데이터베이스 용어들의 쉬운 정의입니다.

* **트랜잭션 (Transaction)**:
  * **어려운 정의**: 데이터베이스의 상태를 변환시키는 하나의 논리적 기능을 수행하기 위한 작업의 단위 또는 한꺼번에 모두 수행되어야 할 일련의 연산들.
  * **쉬운 비유**: **"전부 성공 혹은 전부 실패 (All-or-Nothing) 계약"**입니다. 100개의 백업 거래 기록을 복원하는 중에 99번째 데이터에서 오류가 났을 때, 앞의 98개만 어설프게 복원되어 있으면 전체 장부가 꼬이게 됩니다. 트랜잭션은 100개가 모두 다 완벽하게 입력되거나, 단 하나라도 도중에 에러가 나면 아예 처음부터 아무 일도 없었던 초기 상태로 원상복구(Rollback)시켜 데이터가 뒤엉키는 일을 철저히 예방해 줍니다.

---

## 4. 실전 개발자 가이드: iPad 공유창 크래시 주의보 🚨
