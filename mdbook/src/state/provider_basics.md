...
이를 해결하기 위해 등장한 **Provider**는 상태 객체를 트리 상단에 띄우고 아래의 필요한 위젯들만 쏙쏙 골라 구독할 수 있게 돕는 가장 대중적인 상태 관리 패키지입니다.
...
| :--- | :--- | :--- | :--- |
| **`context.watch<T>()`** | 대상의 데이터가 바뀌면 이 메서드가 포함된 **위젯 전체를 다시 빌드**합니다. | `build` 메서드 안에서 전체 화면을 갱신해야 하는 데이터를 읽을 때 | `initState()`, 클릭 이벤트 핸들러 내부 |
| **`context.read<T>()`** | 대상 데이터를 읽어오기만 하며, 데이터가 바뀌어도 **리빌드를 일으키지 않습니다**. | `onTap` 같은 이벤트 핸들러 함수 내부, `initState` 내 초기화 로직 | **`build` 메서드 바로 밑에서 화면 출력용 데이터 읽기** (값 갱신 불가!) |
| **`context.select<T, R>()`** | 위젯 전체 필드 중 **특정 프로퍼티 `R`만 관찰**하여 해당 값만 변경될 때 리빌드합니다. | 거대한 뷰모델의 특정 일부 정보(예: `isLoading`)만 갱신하는 텍스트나 로딩 위젯 | - |

---

## ⚡ Selector 위젯을 활용한 정밀 렌더링 최적화

`context.select` 보다 한층 더 렌더링 스코프(범위)를 좁게 통제하고 싶을 때 **`Selector` 위젯**을 사용합니다. 
...
> [!WARNING]
> **초보자가 가장 많이 범하는 2대 Provider 버그!**
> 1. **버그 1: `build` 메서드 안에 `context.read` 도배하기**
>    "리빌드가 안 일어나니 성능 향상이 되겠지?" 라며 build 안에 `read`를 써두면, 데이터가 바뀌어도 화면이 전혀 업데이트되지 않아 앱이 멈춘 것처럼 보입니다.
> 2. **버그 2: 클릭 리스너(`onPressed`)에 `context.watch` 넣기**
